# ActiveMQ Artemis
# Adapted from https://github.com/Jacob-/activemq-artemis-docker
##########################################################
## Build Image                                           #
##########################################################
FROM openjdk:11.0.14-jdk as builder
...
RUN sed -i "s/JAVA_ARGS=\"/JAVA_ARGS=\"-Djava.net.preferIPv4Addresses=true -XX:+UnlockExperimentalVMOptions -XX:+UseContainerSupport -XX:MaxRAMPercentage=50 /g" ../etc/artemis.profile
...
# add user and group for artemis
RUN addgroup --gid 1000 --system artemis && adduser -u 1000 --system -G artemis artemis
